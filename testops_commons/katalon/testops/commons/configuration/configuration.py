...
class TestOpsConfigurationCreator(ConfigurationCreator):
    ...
    def create_configuration(self) -> Configuration:
        ...
        server_url: str = param_helper.get_parameter(constants.TESTOPS_SERVER_URL)
        if is_blank(server_url):
            server_url = constants.TESTOPS_SERVER_URL_DEFAULT

        report_folder_path: str = param_helper.get_parameter(constants.TESTOPS_REPORT_DIRECTORY)
        ...
        build_label: str = param_helper.get_parameter(constants.TESTOPS_BUILD_LABEL)
        build_url: str = param_helper.get_parameter(constants.TESTOPS_BUILD_URL)

        pj_id_str: str = param_helper.get_parameter(constants.TESTOPS_PROJECT_ID)
        project_id: int = -1
